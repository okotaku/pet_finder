...
def find_duplicates_all():
    ...
    sims = np.array(
        [(train_hashes[i] == test_hashes).sum(dim=1).cpu().numpy() for i in range(train_hashes.shape[0])]) / 256
...
def run_cat_model(X_train, y_train, X_valid, y_valid, X_test,
                  predictors, maxvalue_dict, fold_id, params, model_name):
...
if __name__ == '__main__':
    ...
    with timer('common features'):
        ...
        with timer('preprocess metadata'):  # 使ってるcolsがkaeruさんとtakuokoで違う kaeruさんがfirst系は全部使うが、takuokoは使わない
            ...
            k_cols += [c for c in meta_parser.g1.columns if re.match("\w*_mean_\w*mean", c)] + ["magnitude_mean",
                                                                                                "score_mean"]
            t_cols += [c for c in meta_parser.g1.columns if re.match("\w*_sum_\w*mean", c)] + ["magnitude_mean",
                                                                                               "score_mean"]
            g_cols += list(meta_parser.g1.columns)

            k_cols += [c for c in meta_parser.g2.columns if
            ...
            g_cols += [c for c in meta_parser.g2.columns if
    ...
    if T_flag:
        ...
        with timer('takuoko feature info for NN'):
            ...
            logger.info(f'predictors / use_cols = {len(use_cols)} / {len(use_colsnn)}')
            ...
            train_cp = train[
                img_cols + numerical + important_numerical + categorical_features + important_categorical].copy()
            for c in categorical_features + important_categorical:
    ...
    if G_flag:
        ...
        with timer('gege cat modeling'):
            y_pred_g_cat = np.empty(len(extract_idx), )
            y_test_g_cat = []
            train_losses, valid_losses = [], []
            ...
            for fold_id, (train_index, valid_index) in enumerate(cv.split(range(len(X)), y=y, groups=rescuer_id)):
                ...
                pred_val, pred_test, train_rmse, valid_rmse = run_cat_model(X_train, y_train,
                                                                            X_valid, y_valid, X_test,
                                                                            predictors, maxvalue_dict,
                                                                            fold_id, CAT_PARAMS, MODEL_NAME + "_g_cat")
                y_pred_g_cat[valid_index] = pred_val
                y_test_g_cat.append(pred_test)
                train_losses.append(train_rmse)
                valid_losses.append(valid_rmse)

            y_test_g_cat = np.mean(y_test_g_cat, axis=0)
            logger.info(f'train RMSE = {np.mean(train_losses)}')
            logger.info(f'valid RMSE = {np.mean(valid_losses)}')

        np.save("y_test_g_cat.npy", y_test_g_cat)
        np.save("y_oof_g_cat.npy", y_pred_g_cat)

        with timer('gege lgb modeling'):
            y_pred_g_lgb = np.empty(len(extract_idx), )
            y_test_g_lgb = []
            train_losses, valid_losses = [], []
            ...
            for fold_id, (train_index, valid_index) in enumerate(cv.split(range(len(X)), y=y, groups=rescuer_id)):
                ...
                pred_val, pred_test, train_rmse, valid_rmse = run_model(X_train, y_train,
                                                                        X_valid, y_valid, X_test, 
                                                                        categorical_features_g, predictors,
                                                                        maxvalue_dict, fold_id, MODEL_PARAMS,
                                                                        MODEL_NAME + "_g_lgb")
                y_pred_g_lgb[valid_index] = pred_val
                y_test_g_lgb.append(pred_test)
                train_losses.append(train_rmse)
                valid_losses.append(valid_rmse)

            y_test_g_lgb = np.mean(y_test_g_lgb, axis=0)
            logger.info(f'train RMSE = {np.mean(train_losses)}')
            logger.info(f'valid RMSE = {np.mean(valid_losses)}')

        np.save("y_test_g_lgb.npy", y_test_g_lgb)
        np.save("y_oof_g_lgb.npy", y_pred_g_lgb)

        with timer('gege lgbm classification modeling'):
            y_pred_class_g = np.empty(len(extract_idx), )
            y_test_class_g = []
            train_losses, valid_losses = [], []
            ...
            for fold_id, (train_index, valid_index) in enumerate(cv.split(range(len(X)), y=y, groups=rescuer_id)):
                ...
                pred_val, pred_test, train_rmse, valid_rmse = run_classifier_model(X_train, y_train, X_valid, y_valid,
                                                                                   X_test, categorical_features_g,
                                                                                   predictors,
                                                                                   fold_id, MODEL_PARAMS_CLASSIFIER,
                                                                                   MODEL_NAME + "_g_class")
                y_pred_class_g[valid_index] = pred_val
                y_test_class_g.append(pred_test)
                train_losses.append(train_rmse)
                valid_losses.append(valid_rmse)

            y_test_class_g = np.mean(y_test_class_g, axis=0)
            logger.info(f'train RMSE = {np.mean(train_losses)}')
            logger.info(f'valid RMSE = {np.mean(valid_losses)}')

        np.save("y_test_class_g.npy", y_test_class_g)
        np.save("y_oof_class_g.npy", y_pred_class_g)
    ...
    if T_flag and K_flag and G_flag:
        with timer('stacking'):
            X = np.concatenate([y_pred_t[extract_idx].reshape(-1, 1),
                                ...
                                y_pred_class_k[extract_idx].reshape(-1, 1),
                                y_pred_g.reshape(-1, 1),
                                y_pred_g_cat.reshape(-1, 1),
                                y_pred_g_lgb.reshape(-1, 1),
                                y_pred_class_g.reshape(-1, 1),
                                ], axis=1)
            X_test = np.concatenate([y_test_t.reshape(-1, 1),
                                     y_test_t_nn.reshape(-1, 1),
                                     y_test_k.reshape(-1, 1),
                                     y_test_class_k.reshape(-1, 1),
                                     y_test_g.reshape(-1, 1),
                                     y_test_g_cat.reshape(-1, 1),
                                     y_test_g_lgb.reshape(-1, 1),
                                     y_test_class_g.reshape(-1, 1),
                                     ], axis=1)
